...
struct RectUnion {
    struct St {
        ...
        int o;
        St() {}
        St(i64 _x1, i64 _x2, i64 _y, int _o) : x1(_x1), x2(_x2), y(_y), o(_o) {}
    };
    int n, tot;
    vector<i64> a, w;
    vector<int> v;
    ...
    int f(i64 x) { return lower_bound(a.begin(), a.begin() + tot, x) - a.begin(); }
    ...
    void add(int lf, int rg, int o, int u = 0, int ul = 0, int ur = -1) {
        if(lf >= rg) return;
        if(ur == -1) ur = tot - 1;
        if(lf == ul && rg == ur) {
            v[u] += o;
    ...
    i64 area() {
        ...
        sort(b.begin(), b.begin() + n * 2, [](const St& i, const St& j) { return i.y < j.y; });
        i64 sum = 0;
